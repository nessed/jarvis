...
| Live queue contents, 2 Sep 2026 | 376 rows: 255 `done`, 103 `dead_letter`, 17 `failed`, 1 `queued`. **`whatsapp_webhook` is 175 rows, all `done`** — the reply path is healthy. **98 of the 103 dead-letters are `distill_memory`**, all between 29 Aug 13:06 and 30 Aug 20:52 UTC: 83 `EmbeddingError`, 12 `LLMError`, 3 `exhausted after stale timeout`. The seven orphaned `queue-durability-probe-` rows (five dead-lettered, two failed, all 25 Aug, empty payloads) are **reported and left in place** per Q9's carve-out, and account for five of the 103 — the premise that they pollute `dead_letter_count` is off by an order of magnitude |
| Distill chain | **Alive but stalled, 2 Sep 2026 — open.** Exactly one `distill_memory` row is `queued` (`dd853e77`) with `run_after` of **30 Aug 20:53**: ripe and unclaimed for over two days, so no `background-worker` has polled since. This settles `docs/audit/blueprint-drift.md` §4's first open question — the chain is neither healthy nor dead-lettered out of existence. Before stalling it died 98 times in ~32 hours, mostly `EmbeddingError`. Ollama is up now and `nomic-embed-text:latest` is installed, so it is not a missing model. `memory/embeddings.py` raises `EmbeddingError` for a timeout, a connect failure and a non-200 alike, so 83 identical checkpoints hide up to three different causes — worth separating before diagnosing. Needs a board task (`distill-chain-stall`); `README.md` was held by another lane when this was found |

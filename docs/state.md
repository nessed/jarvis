...
| Executor topology | The approved implementation is two independently supervised pollers: `whatsapp-worker` is restricted to `whatsapp_webhook`, and `background-worker` is restricted to the long-running `distill_memory` chain; only the latter seeds the chain and maintains the batch heartbeat. The webhook remains enqueue-only. Focused tests pass. **It has run live.** Both workers reached steady-state polling on 31 Aug 2026 — `tools/whatsapp-worker.out.log` (1570 lines) and `tools/background-worker.out.log` (2990 lines), both ending in successful `claim_next_job` calls at 01:53. The earlier "not live: Quick Tunnel startup fails before worker launch" claim is withdrawn; it also contradicted this file's own "WhatsApp voice wiring" row, which records a live-verified voice reply on 30 Aug. Quick Tunnel startup is still fragile (see open blocker 3), but it is no longer blocking worker launch. |

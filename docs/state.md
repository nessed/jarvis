...
5 vision fallback. Phase 3 is underway: `voice-deps-and-tooling` has landed
(openWakeWord, Kokoro, Pipecat, Silero VAD installed and exercised) and the
wake word is **proved working on the pretrained model**. `whisper-npu-build`
is in flight. Phases 4 and 5 have not started.
...
| Executor topology | The approved implementation is two independently supervised pollers: `whatsapp-worker` is restricted to `whatsapp_webhook`, and `background-worker` is restricted to the long-running `distill_memory` chain; only the latter seeds the chain and maintains the batch heartbeat. The webhook remains enqueue-only. Focused tests pass. It is not live: Quick Tunnel startup currently fails before worker launch or Meta verification. |
...
| Startup | Passes `--protocol http2` to cloudflared (`JARVIS_TUNNEL_PROTOCOL` overrides). QUIC is UDP 7844 and is unroutable on this network. `start-jarvis.bat` -> `tools/start_jarvis.py` brings up Ollama, bus, tunnel, Meta re-point and the two workers in order. A Quick Tunnel recovery is required: its initial API call selected an unusable IPv6 address. The launcher accepts only a hyphenated generated `*.trycloudflare.com` hostname, so it ignores the `api.trycloudflare.com` provisioning endpoint in a failure line. Ctrl+C stops the set together; a child dying reports which and shuts the rest down. |
...
| WhatsApp voice wiring | **Built, offline-tested, not yet live-verified.** An inbound voice note now gets a spoken reply, not silence: `executor/handlers/whatsapp.py` downloads it (`WhatsAppClient.download_media()`, Meta's id-then-URL two-step), decodes OGG/Opus to 16 kHz mono PCM WAV (`voice/audio.py`, linear-interpolation resample if the source isn't already 16 kHz — WhatsApp voice notes usually already are), transcribes it (`voice/whisper/server_client.py` against a warm `whisper-server`, language forced to `ur` same as the CLI path), then runs the transcript through the exact same recall/route pipeline a text message uses, and replies with a synthesised voice note (`voice.speak.text_to_voice_note()` + `WhatsAppClient.send_voice_note()`) instead of text — symmetric, per blueprint 3.3. A blank transcript (silence, background noise) is a silent no-op, same treatment an empty-body text message already gets. `tools/start_jarvis.py` now spawns `whisper-server.exe` as a fifth managed process, best-effort: missing NPU build artifacts (machine-local, never committed) log a warning and skip, they do not fail the launch, and text messages are unaffected either way. Every seam (download, decode, transcribe, synthesize, send) is dependency-injected and covered with fakes — no NPU, no Kokoro model, no Graph API needed to run the tests. **Nothing here has been exercised against a real voice note yet** — sending one and confirming the reply by ear is Ali's |
